...
int getPhones(FILE *phoneFile) {
    printf("Creating 100 phones, on each grid point from (0,0) to (9,9)\n");
    int x, y;
    Phone phones[100] = {0};
    int i = 0;
    for (x = 0; x < 10; x++) {
        for (y = 0; y < 10; y++) {
            char idString[10];
            sprintf(idString, "555-%02d%02d", x, y);
            phones[i] = createPhone(idString, x, y);
            if (!phones[i]) {
                printf("FAIL: unable to create phone.\n");
                return 0;
            }
            i++;
        }
